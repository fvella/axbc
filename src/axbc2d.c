...
uint64_t degree_redution_time = 0;
...
void writeStats()
{
	...
	*/
	...
	if (loc_count>0)
		fprintf(fout,"N.\tID\tDegree\tLevel\tVisited\tTEdge\tTEPS\tTot_T\tTot_CUDA\tTot_MPI\tUpw_C\tUpw_M"
			     	 "\tDep_C\tDep_M\tUpd_C\tUpd_M\tOverL\t\tNFRT\n");
	else fprintf(fout,"No vertex processed!");

	for (i = 0; i < loc_count; i++) {
			// Count, Vertex id, Degree, Level, Visited, Tot_Time, Upw_Cuda, Upw_Mpi, Dep_Cuda, Dep_Mpi, Upd_Cuda, Upd_Mpi, OverLap
			fprintf(fout,"%d\t%"PRIu64"\t%d\t%d\t%"PRIu64"\t%"PRIu64"\t%lf\t%"PRIu64
			 ...
			 )
{
	...
	mystats[loc_count].ned = ned;
	mystats[loc_count].teps = teps;
...
static uint64_t norm_graph(uint64_t *ed, uint64_t ned, LOCINT *deg) {
	...
	for(n = l = 1; n < ned; n++) {
		if (deg != NULL) deg[GI2LOCI(ed[2*n])]++;
...
#ifdef _TIMINGS
        //TIMER_STOP(0);
        //prstat((uint64_t)TIMER_ELAPSED(0), "\nexchange_vert time (us):", 1);
        //uint64_t bytes=R*nfrt*sizeof(*frt);
        //prstat(bytes, "exchange_vert sent bytes:", 1);
#endif
	return;
}
...
#ifdef _TIMINGS
        //TIMER_STOP(0);
        //prstat((uint64_t)TIMER_ELAPSED(0), "exchange_horiz time (us):", 1);
        //uint64_t bytes=0;
        //for(i = 0; i < C; i++) bytes += snum[i]*sizeof(*sbuf);
        //prstat(bytes, "exchange_horiz sent bytes:", 1);
#endif
...
static double bc_func(LOCINT *row, LOCINT *col, LOCINT *frt_all, LOCINT* frt, int* hFnum,
					  ...
					  uint64_t* total_time, uint64_t* compu_time, uint64_t* commu_time,
					  int dump) {
...
POP_RANGE
	...
	LOCINT all = 0;
PUSH_RANGE("update bccol0",4);
	if (mycol == 0){
PUSH_RANGE("update bccol0:loop",1);
...
POP_RANGE
	...
	*total_time = TIMER_ELAPSED(0);
	...
	upStats(v0, nvisited, level, msgSize,
				...
				depc_time, depm_time,
				updc_time, updm_time,
				overlap_time, *compu_time, *commu_time,
				*total_time, ned, teps);
#endif
...
PUSH_RANGE("BFS UPWARD",5)
	while(1) {
		...
		//fprintf(outdebug, "BFS_LEVEL=%d\n", level);
		/*get_frt(frt);
		dump_uarray2(frt, row_pp, "FRT-1");
		get_cbuf(cbuf);
		dump_uarray2(cbuf, row_pp, "CBUF-1");
		dump_uarray2(tlvl, MAX_LVL, "TLVL-1");
        */
        ...
        /*
		get_frt(frt);
		dump_uarray2(frt, row_pp, "FRT-2");
		get_cbuf(cbuf);
		dump_uarray2(cbuf, row_pp, "CBUF-2");
		dump_uarray2(tlvl, MAX_LVL, "TLVL-2");
        */
		// if (myid == 0) fprintf(stdout, "[%7.3lf%%] %"PRIu64"\n", (n*100.0)/N, n);
...
static void print_stats(double *teps, int n) {
	...
	nteps = (double*)Malloc(n*sizeof(double));
	//memset(nteps, 0, n*sizeof(double));
...
void usage(const char *pname) {
	...
	prexit("Usage:\n"
			"\t %1$s -p RxC [-d dev0,dev1,...] [-o outfile] [-DdmUR] [-r first_node] [-N <# of searches>]\n"
...
#undef CHECKRTYPE
	...
	if ((debug==1)) {
	  char     fname[MAX_LINE];
	  snprintf(fname, MAX_LINE, "%s_%d.log", "debug", myid);
	...
	// Disable random in sub-clustering
	if (gntask != ntask) random = 0;
	...
	if (gmyid == 0) {
		...
		fprintf(stdout, "Number of bc rounds: %d\n", nbfs);
	...
	if (heuristic == 1) {
		// Remove duplicate edges loops and so on
	...
	if (heuristic == 1) {
		...
		//PUSH_RANGE("degree reduction",1)
		ned = degree_reduction(myid, ntask, &edge, ned, &rem_edge, &rem_ed);
		//POP_RANGE
		TIMER_STOP(0);
		degree_redution_time = TIMER_ELAPSED(0);
	}
    // 2-D PARTITIONING
	...
	if (ntask > 1) {
		...
		// Partition removed edges
		if (heuristic == 1) rem_ed = part_graph(myid, ntask, &rem_edge, rem_ed, 2);
	}
	TIMER_STOP(0);
...
#ifndef _LARGE_LVERTS_NUM
	if (ned > UINT32_MAX) {
		fprintf(stderr,"[%d] Too many vertices assigned to me, %"PRIu64" (max=%u)\n", myid, ned, UINT32_MAX);
		exit(EXIT_FAILURE);
	}
#endif
	if (myid == 0) fprintf(stdout, "task %d: Verifying partitioning...\n", gmyid);
	TIMER_START(0);
	for(n = 0; n < ned; n++) {
    ...
    if (heuristic == 1){
    	if(myid==0) printf("task %d Init bc... %d \n",gmyid, rem_ed);
    ...
    mystats = (STATDATA*)Malloc(nbfs*sizeof(STATDATA));
    memset(mystats, 0, nbfs*sizeof(STATDATA));
#endif
    ...
    // Allocate teps array
    teps = (double*)Malloc(nbfs*sizeof(double));
    memset(teps, 0, nbfs*sizeof(double));
	...
	for(ui = color; ui < nbfs; ui += cntask) {
		...
		/* if (random==1) v0 = select_root2(col);
		else v0 = startv + ui;
		*/

		skip = 0;
		//bc_time = 0; compu_time=0; commu_time=0;
		reach_v0 = 0;
		...
		if (!mono) {
			teps[ui]=
					bc_func(row, col,  frt_all, frt,  hFnum, msk, lvl, deg,  sigma, frt_sigma, delta, rem_ed,
				         	  v0, vRbuf,  vRnum, hSbuf, hSnum, hRbuf, hRnum, hSFbuf, hRFbuf, reach,
							  vrequest, hrequest, status, &bc_time, &compu_time, &commu_time, 0);
		} else {
			//teps[ui]=
					bc_func_mono(row, col,  frt_all, frt,  hFnum, msk, lvl, deg,  sigma, frt_sigma, delta, rem_ed,
							       v0, vRbuf,  vRnum, hSbuf, hSnum, hRbuf, hRnum, hSFbuf, hRFbuf, reach,
	...
	if (myid == 0) {
		...
		fprintf(stdout,"TaskId\tSkip\tRounds\tExecTime\tRoundsTime\tCUDATime\tMPITime"
				       "\t\tMax\t\tMin\t\tMean\t\t1-dRedu\n");
		fprintf(stdout,"%d\t%d\t%d\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n",
				 	 	 	 	 	 	 	 gmyid, skipped, nrounds,
				                             (all_time+bcred_time)/1.0E+6,
											 all_time/1.0E+6,
											 compu_all_time/1.0E+6,
											 commu_all_time/1.0E+6,
											 max_time/1.0E+6,
											 min_time/1.0E+6,
											 all_time/1.0E+6/nrounds,
											 degree_redution_time/1.0E+6);
		...
		fprintf(stdout,"task %d BC rounds CUDA time: %lf sec\n",gmyid,compu_all_time/1.0E+6);
		fprintf(stdout,"task %d BC rounds MPI time: %lf sec\n",gmyid,commu_all_time/1.0E+6);
		...
		if ( heuristic == 1 ){
			fprintf(stdout,"task %d 1-Degree reduction: %lf sec\n",gmyid, degree_redution_time/1.0E+6);
		}

		print_stats(teps, nbfs);
